...
export async function POST(request: NextRequest) {
  try {
    const { user, pharmacyId } = await requirePharmacyProfile()
    ...
    const doc = await addDocument(
      { kind: 'PHARMACY', pharmacyProfileId: pharmacyId, userId: user.id },
      parsed.data,
    )
    return ok(doc, 201)
  } catch (error) {
    return handleApiError(error, 'pharmacy/documents POST')
  }
}
